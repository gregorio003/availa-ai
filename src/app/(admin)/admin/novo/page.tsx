...
export default function NovoClientePage() {
        ...
        <div className="bg-white dark:bg-gray-900 border border-green-300 dark:border-green-800 rounded-2xl p-6 shadow-sm">
          ...
          <div className="bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-lg p-4 space-y-2 text-sm">
            ...
            <div className="flex justify-between gap-4">
              ...
              <span className="text-gray-900 dark:text-white font-mono">{done.password}</span>
            </div>
          </div>
          <button
            onClick={() => {
            ...
            className="mt-4 w-full flex items-center justify-center gap-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-800 dark:text-white text-sm font-medium py-2.5 rounded-lg transition"
          >
  ...
  return (
    <div className="max-w-md">
      <Link
        href="/admin"
        className="flex items-center gap-2 text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white text-sm mb-6 transition"
      >
        <ArrowLeft className="w-4 h-4" /> Voltar
      </Link>
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">Novo cliente</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Senha temporária</label>
          <div className="flex gap-2">
            <input value={form.password} onChange={(e) => set('password', e.target.value)} required className={inputCls + ' font-mono'} />
            <button
              type="button"
        ...
        <div>
          <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Segmento</label>
        ...
        <div className="flex gap-3">
          <div className="flex-1">
            <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Comissão (%)</label>
            <input type="number" min={0} step={0.5} value={form.commission_pct} onChange={(e) => set('commission_pct', e.target.value)} className={inputCls} />
          </div>
          <div className="flex-1">
            <label className="block text-sm text-gray-600 dark:text-gray-400 mb-1">Válido até</label>
            <input type="date" value={form.subscription_valid_until} onChange={(e) => set('subscription_valid_until', e.target.value)} className={inputCls} />
          </div>
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full flex items-center justify-center gap-2 bg-green-600 hover:bg-green-500 disabled:bg-gray-300 dark:disabled:bg-gray-700 text-white font-semibold py-2.5 rounded-lg text-sm transition"
        >
          {loading ? (
            <>
